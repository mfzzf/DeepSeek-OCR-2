...
Start a local server with OpenAI-compatible endpoints:
```Shell
cd DeepSeek-OCR2-master/DeepSeek-OCR2-vllm
python openai_api_server.py --host 0.0.0.0 --port 8000 --model deepseek-ai/DeepSeek-OCR-2
```
Or run with Docker (requires NVIDIA Container Toolkit):
```Shell
docker build -t deepseek-ocr2-openai .
docker run --gpus all -p 8000:8000 --shm-size 4g deepseek-ocr2-openai
```
Then call it using the OpenAI Python SDK:
```python
import base64
from openai import OpenAI

client = OpenAI(base_url="http://localhost:8000/v1", api_key="EMPTY")
image_b64 = base64.b64encode(open("your_image.jpg", "rb").read()).decode()
